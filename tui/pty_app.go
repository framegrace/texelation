...
// PTYApp now uses our own custom parser.
type PTYApp struct {
	title   string
	command string
	cmd     *exec.Cmd
...
func (a *PTYApp) Render() [][]Cell {
	...
	// Get cursor state from our VTerm
	...
	for y := 0; y < rows; y++ {
		...
		for x := 0; x < cols; x++ {
			...
			// Draw the cursor by reversing the cell's colors
...
// NewPTYApp creates an app that will run the given command.
...
// Run starts the command in a PTY and begins feeding its output to our parser.
