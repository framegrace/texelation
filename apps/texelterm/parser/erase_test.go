	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(h *TestHarness) {
				// Place text on multiple lines
				h.SendSeq("\x1b[H") // Home
				h.SendText("Line 0: AAAA")
				h.SendSeq("\r\n")
				h.SendText("Line 1: BBBB")
				h.SendSeq("\r\n")
				h.SendText("Line 2: CCCC")
				h.SendSeq("\r\n")
				h.SendText("Line 3: DDDD")
				// Position cursor at row 2, col 8 (after "Line 2: ")
				h.vterm.SetCursorPos(2, 8)
			},
			seq: "\x1b[J", // Default is 0
			verify: func(t *testing.T, h *TestHarness) {
				// Rows 0 and 1 should be unchanged
				h.AssertText(t, 0, 0, "Line 0: AAAA")
				h.AssertText(t, 0, 1, "Line 1: BBBB")

				// Row 2 col 0-7 should be unchanged
				h.AssertText(t, 0, 2, "Line 2: ")

				// Row 2 col 8 onwards should be blank
				h.AssertBlank(t, 8, 2)
		...
		},
		{
			...
			setup: func(h *TestHarness) {
				...
				h.SendSeq("\r\n")
				h.SendText("CCCCC")
				h.vterm.SetCursorPos(2, 2)
			},
			seq: "\x1b[0J",
			verify: func(t *testing.T, h *TestHarness) {
				h.AssertText(t, 0, 0, "AAAAA")
				h.AssertText(t, 0, 1, "BBBBB")
				h.AssertText(t, 0, 2, "CC")
				h.AssertBlank(t, 2, 2)
			},
		},
		{
			...
			setup: func(h *TestHarness) {
				...
				h.SendSeq("\r\n")
				h.SendText("DDDDD")
				h.vterm.SetCursorPos(2, 5) // Middle of "CCCCCCCC"
			},
			seq: "\x1b[1J",
			verify: func(t *testing.T, h *TestHarness) {
				...
				h.AssertText(t, 6, 2, "CC")

				// Row 3 should be unchanged
				h.AssertText(t, 0, 3, "DDDDD")
			},
		},
		{
		...
		},
		{
			...
			setup: func(h *TestHarness) {
				...
				// Fill some more lines
				for i := 1; i < 24; i++ {
					h.SendSeq("\r\n")
					h.SendText("Line")
				}
				h.vterm.SetCursorPos(23, 3) // After "Lin" on last line
			},
			seq: "\x1b[J",
			verify: func(t *testing.T, h *TestHarness) {
				// First line unchanged
				h.AssertText(t, 0, 0, "First line")
				// Last line partially erased
				h.AssertText(t, 0, 23, "Lin")
				h.AssertBlank(t, 3, 23) // After "Lin" is blank
			},
		},
	}
