...
func TestBufferCacheSetPaneFlags(t *testing.T) {
	...
	var id [16]byte
	id[0] = 9
	pane := cache.SetPaneFlags(id, true, true, 7, true)
	...
	cache.SetPaneFlags(id, false, true, -3, false)
	if pane.Active || !pane.Resizing || pane.ZOrder != -3 || pane.HandlesSelection {
		t.Fatalf("expected active=false resizing=true z=-3 handles=false, got %+v", pane)
	}
}
